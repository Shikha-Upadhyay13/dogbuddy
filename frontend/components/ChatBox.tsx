...
export default function ChatBox() {
  ...
  // Active thread id is owned by the URL (?thread=...) so the Sidebar
  // (separate component) can switch the active chat by navigating.
  // Default = today's natural day thread; "+ New chat" sets a new
  // ?thread param via the router.
  const router = useRouter();
  const searchParams = useSearchParams();
  const urlThread = searchParams?.get("thread") ?? null;
  const threadId = urlThread || (user ? threadIdFor(user.id) : "");
  ...
  // Save on every messages change once we've hydrated. Notify the
  // Sidebar so its chat-history list refreshes in the same tab.
  useEffect(() => {
